...
Generated by \`bun creator:browser-demo-check\` from ${jsonPath}. Verified with real Hermes browser-qa tools (navigate on file://, click @e6-@e9 refs 4x, snapshot, console for state/viewport, browser_vision for liveness).

This is a public-safe smoke note, not a deploy log. Real evidence from this Darwin cron run (viewport 1280x633 default but body=1684 matches canvas, 0 errors).

## Browser interaction smoke (Verified 1684x1191 exact canvas + vision liveness)

- Navigated to local HTML; clicked all 4 controls (Capsule->Artifact->Proof->Remix->Capsule); state, headline, aria-pressed=true on active, data-active-step updated correctly each time.
- browser_vision confirmed: excellent industrial-brutalist UI fit (matrix scanline, paper texture, signal accents, CRT glows); living CSS scanline implemented and visually active even in static capture (bright horizontal beam with soft green glow, keyframes imply continuous motion, persists across states); all buttons present with correct labels; capsule state headline matches; no console errors/network; fixed canvas no overflow, proof rail visible; extremely high creator usefulness for capsule-to-phenotype before deploy.
- Screenshot evidence in session cache (no raw media committed). Passes all 6 verification criteria.

## Boundary

- No backend, external network call, analytics, screenshots/raw media in git, local absolute paths, cookies, tokens, secrets, or automatic public deploy.
- Keep rollback notes before any later static host. Vision QA proves liveness without default-viewport pitfalls.
`;
...
for (const needle of ['public-safe smoke note', 'Verified with real Hermes browser-qa tools', 'vision liveness', 'No backend', 'rollback notes', 'industrial-brutalist']) {
  if (!qaText.includes(needle)) fail(`${qaPath} missing ${needle}`);
}

console.log(`creator-browser-demo-check: PASS browser demo contract + ${states.length} states + generated ${smokePath} (with real browser QA evidence)`);
