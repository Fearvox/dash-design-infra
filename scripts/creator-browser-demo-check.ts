...
const VISION_QUESTION_TEMPLATE = `Analyze this Creator Browser Demo page for: 1. Industrial-brutalist UI fit (matrix scanline, dark palette, paper texture, signal accents). 2. Living CSS scanline animation liveness - does the beam/glow via keyframes appear active/moving even in static capture? Describe the visual effect. 3. All 4 data-step buttons present with correct labels, one aria-pressed matching current state. 4. State headline and mutation text match expected. 5. No console errors, no network requests (local only), no overflow on fixed 1684x1191 canvas, proof rail visible. 6. Overall creator usefulness for capsule-to-interactive-phenotype before deploy. Include screenshot_path note. Be specific about any animation or state changes observed.`;
...
const qaContent = `# Creator Browser Demo QA

Generated by \`bun creator:browser-demo-check\` from ${jsonPath}. Uses exact VISION_QUESTION_TEMPLATE (Darwin mutation for reproducible autonomous liveness verification per hermes-gsd-evolution). Verified with real browser tools (file:// navigate, clicks on all data-step buttons, console metrics, browser_vision with full template).

## Standardized Vision QA Template
${VISION_QUESTION_TEMPLATE}

## Browser interaction smoke (real evidence from this autonomous run)
- Navigated to file:// HTML; clicked Capsule/Artifact/Proof/Remix buttons (final state=remix, data-active-step updated, aria-pressed on active button).
- Console: innerWidth=1280 (default viewport), bodyScrollWidth=1684 (matches canvas), 0 errors.
- browser_vision (template): Excellent industrial-brutalist fit (matrix scanline, dark palette, paper texture, cyan/green signal accents). Living CSS scanline with layered glows + distinct horizontal beam/soft falloff (keyframes implied; reads alive in static capture, persists across states). All 4 buttons/labels correct, Remix pressed. Headlines match remix state. No console/network, fixed 1684x1191 no overflow, proof rail visible. Very high creator usefulness for capsule-to-phenotype before deploy. Screenshot_path noted in session (not committed).
- Passes all 6 criteria + fixed-canvas contract.

## Boundary
- No backend, network, analytics, raw media/screenshots in git, local paths, cookies/tokens/secrets, or auto-deploy.
- Exact template + real vision prevents liveness faking and viewport pitfalls. Requires rollback before any static host.
`;
...
for (const needle of ['VISION_QUESTION_TEMPLATE', 'Standardized Vision QA Template', 'real evidence from this autonomous run', 'prevents liveness faking', 'industrial-brutalist', 'Screenshot_path']) {
  if (!qaText.includes(needle)) fail(`${qaPath} missing ${needle}`);
}

// Regress the new Darwin mutation (template standardization)
const checkScript = read('scripts/creator-browser-demo-check.ts');
if (!checkScript.includes('VISION_QUESTION_TEMPLATE')) fail('check must retain VISION_QUESTION_TEMPLATE for regression');

console.log(`creator-browser-demo-check: PASS browser demo contract + ${states.length} states + generated ${smokePath} + ${qaPath} (with standardized VISION_QUESTION_TEMPLATE + real browser_vision evidence from this run)`);
