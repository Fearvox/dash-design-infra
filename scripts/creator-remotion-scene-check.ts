...
for (const deliverable of [htmlPath, artifactPath, summaryPath, '/tmp/dash-creator-remotion-scene.pdf']) {
...
for (const command of ['creator:remotion-scene-check', 'measure:check', 'print:render', 'browser visual QA', 'security:scan', 'hackathon:score', 'typecheck', 'docs:links']) {
...
console.log(`creator-remotion-scene-check: PASS Remotion scene contract + ${sequences.length} sequences + generated ${artifactPath}`);
