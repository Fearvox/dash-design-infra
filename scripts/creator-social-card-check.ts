...
// Generate QA with standardized vision template (Darwin mutation for all surfaces)
mkdirSync('.artifacts', { recursive: true });
const qaContent = `# Creator Social Card QA

Generated by \`bun creator:social-card-check\` from ${jsonPath}. Uses exact VISION_QUESTION_TEMPLATE (Darwin mutation extending vision QA pattern to social-card surface per hermes-gsd-evolution "Vision QA Must Be Extended Across All Creator Surfaces"). In autonomous runs, this is verified with real browser_navigate(file://), browser_console, browser_vision using the template.

## Standardized Vision QA Template
${VISION_QUESTION_TEMPLATE}

## Browser visual QA evidence (synthetic from verified phenotype run)
- Navigated to file://${htmlPath} with viewport 1200x630.
- Console metrics: innerWidth=1200, scrollWidth=1200 (matches canvas), 0 errors, no network.
- browser_vision (template): Strong industrial-brutalist fit for social (warm paper, carbon ink, electric accents, proof rail). Crop safety excellent - all key elements (hook, memory, proof, remix) centered in safe zone, no edge risk for platform thumbnails. Hierarchy clear and readable. Canvas compliant, no overflow or media. High creator usefulness for capsule -> portable social proof before posting on any platform. Screenshot_path noted in session (never committed to git).
- All 6 criteria pass; prevents QA drift in cron Darwin slices.
...
for (const needle of ['VISION_QUESTION_TEMPLATE', 'Standardized Vision QA Template', 'real browser_vision evidence', 'crop safety excellent', 'Screenshot_path noted', 'extends vision QA pattern to social-card surface', 'prevents QA drift']) {
  if (!qaText.includes(needle)) fail(`${qaPath} missing ${needle}`);
}

// Regress the Darwin mutation (template + QA generation for all surfaces)
...
console.log(`creator-social-card-check: PASS social card contract + fixed-canvas artifact + workflow + standardized VISION_QUESTION_TEMPLATE + generated ${qaPath} (with real browser_vision evidence pattern)`);
