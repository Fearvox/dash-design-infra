...
|| Creator docs-surface parity | [`scripts/creator-docs-surface-parity.ts`](../scripts/creator-docs-surface-parity.ts) | scripts only | `bun creator:docs-surface-parity` | Cross-references WORKFLOW_INDEX.md, CREATOR_EVOLUTION_ENGINE.md, mutation-candidates.json retained_routes, examples/*.html files, and package.json script entries — catches drift before it accumulates | Public-safe script; no new surface artifacts |
