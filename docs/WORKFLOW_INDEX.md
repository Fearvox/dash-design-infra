...
|||| Creator surface consistency | [`scripts/creator-surface-consistency.ts`](../scripts/creator-surface-consistency.ts) | scripts only | `bun creator:surface-consistency` | Visual grammar audit across all 12 creator surfaces: extracts CSS custom properties from `:root` blocks, identifies visual families (warm-paper, dark-industrial, creative-hybrid), cross-references token names/values, reports naming inconsistencies and value drift. Now includes value-level convergence tracking: per-diverged-token distinct value counts (e.g., `--line` has 10 distinct values across 11 surfaces), convergence delta computation from prior run JSON baseline (`examples/creator-surface-convergence.json`), per-token convergence/divergence/stall trend icons, and a MECHANICAL DRIFT detector that surfaces whitespace/leading-zero/case formatting differences functionally identical to majority consensus (zero-risk alignment targets). Additive to existing audit; no runtime inflation. Exit 0 for informational audit, exit 1 if any surface lacks a CSS token layer. Revealed: 6 shared tokens, 13 diverged, 0 no-token-layer surfaces. Wired into regression orchestrator after cache-integrity. Primary diagnostic tool for NorthStar aesthetic coherence. |
||||| Creator pad-page token check | [`scripts/creator-pad-page-token-check.ts`](../scripts/creator-pad-page-token-check.ts) | scripts only | `bun creator:pad-page-token-check` | Validates warm-paper family `--pad-page` token layer: 3 Pattern A consumers (motion-storyboard, poster-surface, prompt-dna-adapter) declare `--pad-page: 56px` in `:root` and consume `var(--pad-page)` on `.page` padding; family-dna declares `--pad-page: 56px` as the warm-paper visual reference. Catches hardcoded `padding: 56px` regression on the 3 consumers. Wired into regression orchestrator after surface-consistency. Closes the page-geometry diagnostic's addressable Pattern A drift. |
||||| Creator page grid check | [`scripts/creator-page-grid-check.ts`](../scripts/creator-page-grid-check.ts) | scripts only | `bun creator:page-grid-check` | Validates warm-paper page-background grid ink alignment: 4 functional warm-paper .page surfaces (pdf-zine, social-card, motion-storyboard, prompt-dna-adapter) must use `linear-gradient(90deg, rgba(21, 18, 14, alpha) 1px, transparent 1px)` + horizontal grid with exact `--ink` rgb(21,18,14). Catches off-ink drift (e.g. rgba(22,19,14) or rgba(20,17,13)). Per-surface grid alpha (.035/.045/.055/.06) and grid size (42/44/52/56px) retained as content-density tuning (intentional family signature). Extends aesthetic-coherence audit from `:root` tokens to body-CSS rgba patterns. Wired into regression orchestrator after pad-page-token-check. |
|||||| Creator paper-tint stop check | [`scripts/creator-paper-tint-stop-check.ts`](../scripts/creator-paper-tint-stop-check.ts) | scripts only | `bun creator:paper-tint-stop-check` | Validates warm-paper 3-stop diagonal page-tint gradient 1st stop token participation: 3 functional warm-paper surfaces (pdf-zine, social-card, poster-surface) must consume `var(--paper-hot)` as the 1st stop in their `linear-gradient(135deg, A 0%, B NN%, C 100%)` page-tint pattern, declare `--paper-hot: #fff8ea` in `:root`, and have zero residual `--paper-2` references across all `examples/*.html`. 3rd stops (#e4d6c2, #e6dac8, #ddd1bf) intentionally retained as per-surface warm shadow tinting (family signature, same justification as grid alpha/size variation). 2nd stops (var(--paper), #ece0cd) intentionally retained as authored. Third warm-paper body-CSS pattern layer after `--pad-page` (page padding) and `--ink-rgb` (page grid). The `--paper-hot` token now reaches from `:root` declaration into the gradient highlight stop of every warm-paper zine/poster surface. Wired into regression orchestrator after page-grid-check. |
||||| Creator warm-paper spacing pattern check | [`scripts/creator-warm-paper-spacing-pattern-check.ts`](../scripts/creator-warm-paper-spacing-pattern-check.ts) | scripts only | `bun creator:warm-paper-spacing-pattern-check` | Validates the warm-paper page-geometry diagnostic's per-surface pattern classification matches the source HTML: 6 warm-paper .page-bearing surfaces classified into Pattern A (direct page padding) / A+B (page + inner subframe) / B+C (sheet + page::before frame) by inspecting `var(--pad-page)` consumption + `.shell`/`.sheet`/`.poster-frame`/`.capsule-shell` presence + `.page::before` `inset:` frame. Catches drift if a surface changes its `.page` padding source without updating the diagnostic. Verifies the diagnostic HTML contains the Pattern A/B/C visible labels + the hardcoded spacing inventory section (F6) + per-surface consumer documentation. Closes the page-geometry diagnostic's pattern classification axis: 2 Pattern A (family-dna, social-card), 3 A+B (poster, motion-storyboard, prompt-dna), 1 B+C (pdf-zine). Wired into regression orchestrator after page-grid-check. |
...
|||| Creator type size token audit | [`examples/creator-type-size-token-audit.html`](../examples/creator-type-size-token-audit.html) | HTML artifact | `bun measure:check -- examples/creator-type-size-token-audit.html --canvas=1684x1191` and `bun print:render -- examples/creator-type-size-token-audit.html /tmp/dash-type-size-token-audit.pdf --canvas=1684x1191` | Type size token audit: confirms text-size token availability and hardcoded type-scale gaps before any tokenization route. Use `creator:surface-consistency` as the machine gate; no surface edits recommended without design input. Fixed canvas fits. | Public-safe diagnostic artifact; no surface edits |
...
||||| Creator accent family diagnostic | [`examples/creator-accent-family-signature-diagnostic.html`](../examples/creator-accent-family-signature-diagnostic.html) | HTML artifact | `bun measure:check -- examples/creator-accent-family-signature-diagnostic.html --canvas=1684x1191` and `bun print:render -- examples/creator-accent-family-signature-diagnostic.html /tmp/dash-accent-diagnostic.pdf --canvas=1684x1191` | --accent family signature diagnostic: documents #ffb86b warm-paper consensus and intentional cross-family accent divergence. Fixed canvas fits. | Public-safe diagnostic artifact; no surface edits |
||||| Creator family grammar audit | [`examples/creator-family-grammar-audit.html`](../examples/creator-family-grammar-audit.html) | HTML artifact | `bun measure:check -- examples/creator-family-grammar-audit.html --canvas=1684x1191` and `bun print:render -- examples/creator-family-grammar-audit.html /tmp/dash-family-grammar-audit.pdf --canvas=1684x1191` | Warm-paper family grammar audit for accent, line, and panel tokens; records frontier-capsule divergence as retained family signature, not addressable drift. Fixed canvas fits. | Public-safe diagnostic artifact; no surface edits |
...
|||||||||| Creator family-dna spacing grammar audit | [`examples/creator-family-dna-spacing-grammar-audit.html`](../examples/creator-family-dna-spacing-grammar-audit.html) | HTML artifact | `bun measure:check -- examples/creator-family-dna-spacing-grammar-audit.html --canvas=1684x1191` and `bun print:render -- examples/creator-family-dna-spacing-grammar-audit.html /tmp/dash-creator-family-dna-spacing-grammar-audit.pdf --canvas=1684x1191` | Spacing grammar audit: family-dna has 6 var(--space-4) usages (premise stated "0" — factually stale). 4 hardcoded 20px in .family-card/.grammar are intentional card padding (design decision), not grammar drift. Warm-paper spacing pattern: token user for base grid, hardcoded for card padding. No addressable drift. Fixed canvas fits (1684x1361). | Public-safe diagnostic artifact; no surface edits |
||||||||||| Creator warm-paper page geometry diagnostic | [`examples/creator-warm-paper-page-geometry-diagnostic.html`](../examples/creator-warm-paper-page-geometry-diagnostic.html) | HTML artifact | `bun measure:check -- examples/creator-warm-paper-page-geometry-diagnostic.html --canvas=1684x1191` and `bun print:render -- examples/creator-warm-paper-page-geometry-diagnostic.html /tmp/dash-creator-warm-paper-page-geometry-diagnostic.pdf --canvas=1684x1191` | Page geometry audit: 6 warm-paper surfaces show 3 role-distinguished patterns (page padding, inner container, page::before frame). F6 hardcoded spacing inventory cross-references `examples/creator-surface-convergence.json` `spacing_audit` per-surface values with the pattern classification (2 Pattern A, 3 A+B, 1 B+C). F7 pattern↔token status documents that `--pad-page: 56px` token layer has reached saturation (3/3 Pattern A consumers + 1/1 reference = full coverage on the 1684×1191 canvas). Outer padding 44–70px range; inner 40–70px. Token candidates `--pad-shell`, `--pad-frame` identified but require creator/design verdict on whether warm-paper family should be uniform (token + 56px) or rhythm-varying (signature + 3-patterns). Fixed canvas fits. Machine-checkable via `bun creator:warm-paper-spacing-pattern-check`. | Public-safe diagnostic artifact; no surface edits |
||||||||||| Creator warm-paper Pattern B shell padding diagnostic | [`examples/creator-pad-shell-pattern-b-diagnostic.html`](../examples/creator-pad-shell-pattern-b-diagnostic.html) | HTML artifact | `bun measure:check -- examples/creator-pad-shell-pattern-b-diagnostic.html --canvas=1684x1191` and `bun print:render -- examples/creator-pad-shell-pattern-b-diagnostic.html /tmp/dash-pad-shell-pattern-b-diagnostic.pdf --canvas=1684x1191` | Pattern B inner shell padding audit: 4 warm-paper surfaces use inner containers (`.shell`, `.poster-frame`, `.capsule`, `.sheet`) with padding 40–70px. 4px drift between motion-storyboard (40px) and prompt-dna/poster (44px) exists; proportionality test shows motion-storyboard has *higher* chrome-to-padding ratio (0.385) than prompt-dna (0.289) — not proportional to chrome scale. 4 different class names for the same role; only `.poster-frame` is renamable to `.shell`. `--pad-shell` tokenization requires design verdict on motion-storyboard 40px intentionality. `--pad-page: 56px` already shipped (PR #156). Fixed canvas fits. | Public-safe diagnostic artifact; no surface edits |
||||||||||| Creator cross-family grammar audit | [`examples/creator-cross-family-grammar-audit.html`](../examples/creator-cross-family-grammar-audit.html) | HTML artifact | `bun measure:check -- examples/creator-cross-family-grammar-audit.html --canvas=1684x1191` and `bun print:render -- examples/creator-cross-family-grammar-audit.html /tmp/dash-creator-cross-family-grammar-audit.pdf --canvas=1684x1191` | Cross-family grammar audit: 14 diverged tokens across 22 surfaces classified into 4 visual families (warm-paper, dark-industrial, creative-hybrid, mixed/frontier). 13 of 14 are family signatures, family-converged, or surface-only creative content. 1 addressable drift (`--green` on creator-ink-family-diagnostic) is diagnostic-only — zero addressable drift on functional warm-paper surfaces. Closes 5 stale ledger candidates (page-geometry, ink-family, font-sans, type-size, line-color). Fixed canvas fits. | Public-safe diagnostic artifact; no surface edits |
