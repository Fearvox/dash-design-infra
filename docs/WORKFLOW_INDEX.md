...
| Creator regression orchestrator | [`scripts/creator-regression-check.ts`](../scripts/creator-regression-check.ts) | scripts only | `bun creator:regression` | All 15 surface + mutation + capsule + evolution + hackathon checks pass in a single pass with per-surface PASS/FAIL, timing, and aggregate verdict | Public-safe script; no new surface artifacts |
