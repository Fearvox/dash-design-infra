...
- [`creator-ink-token-alignment-route`](../examples/creator-pdf-zine.html): CSS --ink token alignment mutation (PR #73); aligns the `--ink` custom property across the warm-paper family toward consensus `#15120e` — fixes pdf-zine (drift 2 hex digits) + frontier-capsule, p5-sketch creative intent preserved. `--ink` converges from 6→3 distinct values. All gates MAXXED (316/316 hackathon, 11/11 cache-integrity, 4/4 surface checks). Fifth concrete token-alignment step — completes warm-paper ink cohesion for NorthStar creator-surface-consistency.
