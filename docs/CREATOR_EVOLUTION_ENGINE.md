...
- [`creator-regression-noise-route`](../scripts/creator-regression.ts): Regression noise filter stub; deferred — regression currently clean, no pre-existing bun-types lint noise to suppress. May become relevant if future bun-types updates introduce persistent pre-existing noise.
