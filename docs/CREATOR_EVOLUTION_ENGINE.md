...
- [`creator-spacing-grammar-audit-route`](../examples/creator-gap-spacing-audit.html): Gap/spacing grammar diagnostic mutation (PR #102); hardcoded px gap values across 12 surfaces audit confirms: (1) 5 warm-paper surfaces have --space-4 in :root but zero body CSS usage, (2) 6 surfaces (family-dna, frontier-capsule, dark/creative families) lack --space-4 entirely, (3) 16px hardcoded gaps found in poster ×2, family-dna ×1, manim ×1, touchdesigner ×1, (4) gap values 4px-64px range — each deliberate zine-like layout decision, not addressable drift. Diagnostic phenotype: `creator-gap-spacing-audit.html` (1 page, 1684×1191). Next: body adoption of existing --space-4 in warm-paper surfaces.
- [`creator-bg-token-diagnostic-route`](../examples/creator-bg-token-diagnostic.html): --bg family signature diagnostic mutation (PR #95); confirms `--bg` is a dark-industrial family token — not a warm-paper token. Warm-paper surfaces use `--paper` for background. 0 warm-paper surfaces define `--bg`. Family signature confirmed — no addressable drift. No surface edits. Diagnostic phenotype documents --bg presence across all 11 surfaces.
- [`creator-type-size-audit-route`](../examples/creator-type-size-audit.html): Type size diagnostic mutation (PR #100); audits h1 font-size values across all 11 surfaces — finds 10 distinct values with no shared token. --space-4 token missing from family-dna and frontier-capsule. Extends --space-4 to both surfaces. All 11 surfaces now have --space-4 token layer. Diagnostic phenotype documents type size audit findings.
- [`creator-space4-extension-route`](../examples/creator-family-dna.html): --space-4 token extension mutation (PR #101); extends --space-4:16px from family-dna and frontier-capsule :root declarations to all 11 surfaces, adding missing `--space-4:16px;` to :root in frontier-capsule. All 11 surfaces now have spacing token layer. Closes the last --space-4 gap in warm-paper surfaces.
